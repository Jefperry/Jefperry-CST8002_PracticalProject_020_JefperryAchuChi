...
class TestSortingFunctionality(unittest.TestCase):
    ...
    def test_sort_by_site_identification(self):
        ...
        self.assertEqual(
            sorted_records[-1].get_site_identification(), "SITE_C")

        # Verify alphabetical ordering
        for i in range(len(sorted_records) - 1):
            self.assertLessEqual(sorted_records[i].get_site_identification(),
                                 sorted_records[i +
                                                1].get_site_identification(),
    ...
    def test_sort_by_multiple_criteria(self):
        ...
        site_a_records = [
            r for r in sorted_records if r.get_site_identification() == "SITE_A"]
    ...
    def test_get_sorted_copy_non_destructive(self):
        ...
        # Act - get sorted copy
        sorted_copy = self.manager.get_sorted_copy(
            sort_by="year", reverse=False)
    ...
    def test_record_comparison_operators(self):
        ...
        record1 = KelpFishRecord(
            site_identification="SITE_A", year=2010, species_code="FISH_A", count=5)
        record2 = KelpFishRecord(
            site_identification="SITE_A", year=2015, species_code="FISH_B", count=10)

        # Assert - test comparison operators
        self.assertTrue(
            record1 < record2, "Record with earlier year should be less than record with later year")
        self.assertTrue(
            record2 > record1, "Record with later year should be greater than record with earlier year")
